...
class Args:
    ...
    local_update_epochs: int = 8
    """the K local epochs to update the policy per user batch"""
    norm_adv: bool = True
...
class Agent(nn.Module):
    def __init__(self, envs, num_hidden_neurons):
        ...
        self.actor = nn.Sequential(
            ...
            layer_init(nn.Linear(self.num_hidden_neurons, self.num_hidden_neurons)),
            nn.Tanh(),
            layer_init(nn.Linear(self.num_hidden_neurons, envs.single_action_space.n), std=0.01),
        )
...
def train(args):
    ...
    optimizer = optim.Adam(agent.parameters(), lr=1, eps=1e-5) # optim.Adam(agent.parameters(), lr=args.learning_rate, eps=1e-5)
    local_agents = [copy.deepcopy(agent) for _ in range(len(pools))]
    local_optimizers = [optim.Adam(local_agents[i].parameters(), lr=args.learning_rate, eps=1e-5) for i in range(len(local_agents))]
    #local_optimizers = [optim.SGD(local_agents[i].parameters(), lr=args.learning_rate) for i in range(len(local_agents))]
        ...
        for pool_id in range(len(pools)):
            ...
            for epoch in range(args.local_update_epochs):
                ...
                for start in range(0, int(args.num_steps * len(pools[pool_id])), minibatch_size):
                    ...
                    _, newlogprob, entropy, newvalue = local_agent.get_action_and_value(b_obs[mb_inds], b_actions.long()[mb_inds])
                    ...
                    if not args.original_clipping:
                        with torch.no_grad():
                            ...
                            if not args.per_layer_update:
                                ...
                                clip_factor_value = np.max([1, local_update_norm_value.cpu() / (args.max_grad_norm_value)])
                                for name, param in local_agent.named_parameters():
                                    if name.startswith('actor'):
        ...
        with torch.no_grad():
            ...
            for name, param in agent.named_parameters():
                if args.noise_multiplier > 0 and name.startswith('actor'):
                    agg_gradient[name] += torch.normal(mean=0, std=sigma, size=agg_gradient[name].size()).to(device)
    ...
    if args.save_model:
        torch.save(agent.state_dict(), f'./models/dp_fed_ppo_{args.env_id}_z_{int(args.noise_multiplier)}_seed_{args.seed}.pt')
